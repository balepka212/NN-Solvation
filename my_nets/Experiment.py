...
from my_nets.ResNET import ResNet1D, MyConv1dPadSame, MyMaxPool1dPadSame, BasicBlock
...
def Experiment(runs_folder='Example_Lin1',
    ...
    """
    global train_loader, val_loader, solvent_test_loader, solute_test_loader, dataset, model, loss_function
    ...
    if not previous:
        ...
        train_dataset, val_dataset = dataset.train_val_split(len_data - val_data)
        ...
        #     for dl_name, dl in zip(('train', 'val', 'solvent', 'solute'),
        #                            (train_dataset, val_dataset, solvent_test_dataset, solute_test_dataset)):
        #         dl_data[dl_name]['X'] = [pd.DataFrame(), ]
        #     in_feat = next(iter(dataset))[0].shape[0]  # get input tensor size
        #     model = KernelRidge(**net_dict)


        loss_function = nn.MSELoss(reduction='sum')
        Experiment.optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    model.train()
    print('Training')
    MSE = train(model, train_loader, val_loader, solvent_test_loader, solute_test_loader, loss_function,
                Experiment.optimizer, epochs=epochs, save_epochs=save_epochs, ckp_path=runs_folder, start_epoch=start_epoch,
                val_loss_min_input=val_loss_min_input, from_start=not previous, tele_log=tele_log)
    print('Finished training!')
    plot_losses(project_path('Run_results/' + runs_folder + '/run_log.tsv'), save=True)
    return MSE
...
def Continue_experiment(runs_folder='Example_Lin1',
    ...
    try:
        os.makedirs(project_path('Runs/' + runs_folder + '/run_log_hist'))
    except OSError as e:
        if e.errno != errno.EEXIST:
            raise
    ...
    run_folder = project_path('Runs/' + runs_folder)
    try:
    ...
    print('Training')
    MSE = train(model, train_loader, val_loader, solvent_test_loader, solute_test_loader, loss_function,
                Experiment.optimizer, epochs=epochs, ckp_path=runs_folder, start_epoch=start_epoch,
